import { Colors, Config } from "../types/index";
    ...
    for(const [i, part] of [part1, part2].entries()) {
        ...
        for(const test of part.tests) {
            ...
            if (SUT === expected) {
                ...
                console.log(Colors.FgRed, `   ❌ Expected: ${expected}. Got: ${SUT}`, Colors.Reset);
            }
            const endTime = performance.now();
            const elapsed = endTime - startTime;
            console.log(Colors.FgMagenta, `         ⌛︎ Took ${elapsed} ms`, Colors.Reset);
        }
    }
}