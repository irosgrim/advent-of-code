...
const isArrayIncluded = (first, second) => first.every(x => second.includes(x));
const arrIntersection = (first, second) => first.filter(x => second.includes(x));
...
const solve = (inpt: string, part = "first") => {
    ...
    for (let i of arr) {
        ...
        if (part === "first") {
            if(isArrayIncluded(first.range, second.range) || isArrayIncluded(second.range, first.range)) {
                overlapping.push(i);
            }
        } else {
