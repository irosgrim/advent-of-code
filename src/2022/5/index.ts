...
const solve = (inpt: string) => {
    ...
    const input = arr.slice(0, movesIndex).map(x => x.match(/.{1,4}/g)
        .map(y => y[1])
    );
    const moves = arr.slice(movesIndex + 1, arr.length).map(x => x.match(/\d+/g).map(Number));
    ...
    const crates = input[0].map((v, i) => input.map(row => row[i]).filter(x => x !== ' '));
    ...
    const part1 = run("one").map(x => x[0] || '').join("");
    const part2 = run("two").map(x => x[0] || '').join("");
    return {
        firstPart: part1,
        secondPart: part2,
    }
