...
const priority = (str) => {
    ...
    for(let i = 0; i < str.length; i++) {
        ...
        const value = isUpperCase(letter) ? (str.charCodeAt(i) - uppercaseA): str.charCodeAt(i) - lowercaseA;
...
const arr = (inpt: string, part = "part1") => {
    ...
    return strArr.map((x) => {
        ...
        return part === "part1" ? [firstPriority, secondPriority] : {...firstPriority, ...secondPriority};
    });
};
...
const part2 = (inpt: string) => {
    const r = repeatingInGroups(arr(inpt, "part2"), 3).reduce((acc, curr) => acc += curr.value, 0)
    return r;
}
